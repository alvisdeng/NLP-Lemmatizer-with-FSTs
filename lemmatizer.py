...
class Lemmatizer():
  ...
  def get_in_vocab_fst(self):
    ...
    st = fststr.symbols_table_from_alphabet(alphabet) # <class 'pywrapfst.SymbolTable'>
        ...
        if form_word_length >= lemma_word_length:  
            for i in range(form_word_length):
                if i < lemma_word_length:
                    compiler.write(str(i) + ' ' + str(i+1) + ' ' + form_word[i] + ' ' + lemma_word[i] + '\n')
                else:
                    compiler.write(str(i) + ' ' + str(i+1) + ' ' + form_word[i] + ' <epsilon>\n')
            ...
            c_fst = fst.compose(a_fst.arcsort(sort_type="olabel"), b_fst.arcsort(sort_type="ilabel"))
            ...
            for i in range(lemma_word_length):
                if i < form_word_length:
                    ...
                    compiler.write(str(i) + ' ' + str(i+1) + ' <epsilon>' + ' ' + lemma_word[i] + '\n')
...
if __name__ == '__main__':
  L = Lemmatizer()
  print(L.lemmatize('ases'))
  print(L.delemmatize('give+Guess'))
